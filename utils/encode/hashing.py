...
from config.encode_cfg import (
    ALGORITHM, ENCODING_FUNCTION, DEV_SUFFIX_CROP, DEFAULT_SUFFIX_CROP)
...
## DEFINE SUFFIX CROP
def _get_suffix_crop():
    if DEV_ENVIRONMENT in ["DEV", "LOCAL"]:
        return DEV_SUFFIX_CROP
    return DEFAULT_SUFFIX_CROP
...
def make_suffix_from_branch_ids(branch_id_1: bytes, branch_id_2: bytes):
    crop = _get_suffix_crop()
...
def make_lakat_cid_and_serialize(content: any, codec: int, namespace: int, branch_id_1: bytes, branch_id_2: bytes):
    ...
    if branch_id_1 == bytes(0):
        raise ERR_N_HASH_1
    suffix = make_suffix_from_branch_ids(branch_id_1=branch_id_1, branch_id_2=branch_id_2)
